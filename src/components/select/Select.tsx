        ...
        <div>
            ...
            >
                <img
                    ...
                    onClick={handlePlaceHolderClick} // Обработчик клика по стрелочке
                />
                <div
                    className={clsx(styles.placeholder, styles[selected?.optionClassName || ''])}
                    data-selected={!!selected?.value}
                    onClick={handlePlaceHolderClick} // Обработчик клика по плейсхолдеру
                    role='button'
                    tabIndex={0}
                    ref={placeholderRef}
                >
                ...
                {isOpen && (
                    <ul className={styles.select} data-testid='selectDropdown'>
                        {options.filter((option) => selected?.value !== option.value)
                            .map((option) => (
